...
<ul>
    ...
    <li>
        Dosen Pengajar: <strong> <a href="{{ route('dosens.show', ['dosen' => $matakuliah->dosen->id]) }}">{{ $matakuliah->dosen->nama }}</a></strong>
    </li>
    <li>Jurusan:  <strong>{{ $matakuliah->jurusan->nama }}</strong></li>
    <li>Jumlah SKS: <strong>{{ $matakuliah->jumlah_sks }}</strong></li>
    <li>Total Mahasiswa: <strong>{{ count($mahasiswas_sudah_terdaftar) }}</strong></li>
</ul>
<hr>
<h5 class="mt-5 mb-4">Daftar mahasiswa {{ $matakuliah->jurusan->nama }} yang mengambil mata kuliah {{ $matakuliah->nama }}</h5>

<form method="POST" action="{{ route('proses-daftarkan-mahasiswa', ['matakuliah' => $matakuliah->id]) }}">
    @csrf
        ...
        <div class="col-md-12" style="column-count: 3;">
            @foreach ($mahasiswas as $mahasiswa )
                <div class="custom-control custom-checkbox mb-2">
                    <input type="checkbox" class="custom-control-input" name="mahasiswa[]" value="{{ $mahasiswa->id }}" id="mahasiswa-{{ $mahasiswa->id }}"
