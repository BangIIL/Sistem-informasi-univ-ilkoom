...
<ul>
    ...
    <li>Nomor Induk Mahasiswa: <strong>{{$mahasiswa->nim}}</li>
    <li>Jurusan: <strong>{{$mahasiswa->jurusan->nama}} </strong></li>
    <li> Total Mata Kuliah:
        <strong>
            {{ $mahasiswa->matakuliahs->count() }} ({{ $mahasiswa->matakuliahs->sum('jumlah_sks')}} sks)
        </strong>
    </li>
</ul>
<hr>
<h5 class="my-4">Daftar Mata Kuliah {{ $mahasiswa->jurusan->nama }} yang diambil oleh {{ $mahasiswa->nama }}:</h5>
    ...
    <div class="row">
        @error('matakuliah.*')
            <div class="invalid-feedback my-3 d-block">
                <strong>
                    Error: Pilihan mata kuliah ada yang berulang / bukan dari jurusan {{ $mahasiswa->jurusan->nama }}!
                </strong>
            </div>
        @enderror
        <div class="col-md-12">
            @foreach ($matakuliahs as $matakuliah )
                <div class="mb-2">
                    <input type="checkbox" class="form-check-input" name="matakuliah[]" value="{{$matakuliah->id}}" id="matakuliah-{{$matakuliah->id}}"
